...
trades, congressional/officials trading, institutional filings, federal
contract awards, SEC 8-Ks, news, and earnings.
...
| **Direction signal** | Bullish/bearish/neutral with a confidence %, from premium skew weighted toward near-the-money strikes. |
| **Searchable & sortable** | Live client-side search, direction filter, min-score slider, click-to-drill-down. |
| **Auto-update** | Background scheduler rescans on an interval; the UI repolls every 30s. Manual "Scan now" button too. |
| **Catalyst overlay** | Insider (Form 4), congress/officials, institutional (13F), federal contracts, SEC filings, news, earnings — attached to each ticker and shown as a live feed. |
...
| `FMP_API_KEY` | Financial Modeling Prep | ✅ free tier | Extra congress/institutional failover (reserved hook) | https://site.financialmodelingprep.com/developer/docs |
| `NEWSAPI_KEY` | NewsAPI | ✅ free dev tier | Broader news catalysts (reserved hook) | https://newsapi.org/register |
| `QUIVER_API_KEY` | Quiver Quantitative | ✅ limited free | Premium congress / gov-contract / lobbying (reserved hook) | https://www.quiverquant.com/ |
...
positioning when > 1), and **moneyness**. It then blends:

```
flow_score = 55% · log-scaled unusual premium
           + 25% · peak vol/OI ratio
           + 20% · breadth (# of unusual contracts)        → 0-100
```

**Direction** comes from signed premium (calls +, puts −), weighted 1.5× for
near-the-money strikes where conviction concentrates; confidence is
`|net| / gross`. Catalysts add a capped boost (e.g. insider buy +6, congress
+5) so a name with both unusual flow *and* an insider buy ranks above pure flow.
